  ...
  createLambda(role, llamaFile, function(err, data) {
    ...
    if (llamaFile) {
      ...
      configSchedule(interval, llamaConfig.FunctionArn, function(err) {
        ...
        console.log(chalk.green(util.format('+ Chaos Lambda\'s schedule updated to run during business days from 9-15 every %s minutes', interval)));
      });
    }
  });
}
...
function createEventRule(context, next) {
  ...
  var params = {
    ...
    ScheduleExpression: 'cron(0/' + context.interval + ' 9-15 ? * MON-FRI *)',
    State: 'ENABLED'
  };
